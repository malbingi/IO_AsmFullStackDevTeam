...
import graphviz
from packages import CodeAnalyzer
...
def main():
    ...
    for m in CodeReader.Methods.values():
        ...
        comp = CodeAnalyzer.CodeAnalysis(str(CodeReader.CodeReaders.get(m.file_index).filename), m.name)
        if not m.is_class_method:
            ...
            elements = []
            dict_with_relationships[m.name + " - " + str(comp)] = str(CodeReader.CodeReaders.get(m.file_index).filename)
